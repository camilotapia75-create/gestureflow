  ...
  useEffect(() => {
    ...
    function loop(timestamp: number) {
      ...
      if (now - debugTimerRef.current > 500) {
        ...
        setDebugLine(
          `model:${stage} vr:${video.readyState} lm:${lmCount} nulls:${nullStreakRef.current} ${video.videoWidth}×${video.videoHeight} | nose/sh:${state.noseAboveShoulder.toFixed(2)} slouch:${state.isSlouching}`
        );
      }
